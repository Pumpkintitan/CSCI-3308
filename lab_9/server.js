...
app.get('/home/pick_color', function (req, res) {
	...
	var color_options = `select * from favorite_colors where hex_value='${color_choice}'` // Write a SQL query to retrieve the colors from the database
	...
	db.task('get-everything', task => {
		...
		.then(info => {
			res.render('pages/home', {
				...
				color: info[0].name, // Return the color choice
				color_msg: info[1].color_msg, // Return the color message
			})
		})
		.catch(err => {
			console.log('error', err);
	...
	db.task('get-everything', task => {
		...
		.then(info => {
			res.render('pages/home', {
				...
				color: info[1].hex_value,// Return the hex value of the color added to the table
				color_msg: info[1].color_msg,// Return the color message of the color added to the table
			})
		})
		.catch(err => {
			console.log('error', err);
...
app.get('/team_stats', function (req, res) {
	var games = `SELECT * FROM football_games WHERE extract(YEAR FROM game_date) = 2020`
	var wins = `SELECT COUNT(*) AS wins FROM football_games WHERE extract(YEAR FROM game_date) = 2020 AND home_score > visitor_score`
	var losses = `SELECT COUNT(*) AS losses FROM football_games WHERE extract(YEAR FROM game_date) = 2020 AND home_score < visitor_score`
...
app.get('/player_info/post', function (req, res) {
	...
	var playerstats = `SELECT * FROM football_games WHERE ${player_choice} = ANY(players)`
