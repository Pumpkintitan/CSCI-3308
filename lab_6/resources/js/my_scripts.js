...
function loadStatsPage() {
    ...
    for (row_counter = 2; row_counter < table.rows.length; row_counter++) {
        ...
        for (col_counter = 2; col_counter < table.rows[row_counter].cells.length; col_counter++) {
            ...
            cell_value = parseInt(cell_value); //Increase the cell's value by 2
            if (col_counter == 2) {
                cutemp = cell_value;
            } else if (col_counter == 3) [ (ncutemp = cell_value) ];
        }
        if (cutemp > ncutemp) {
            cu_wins++;
        } else {
    ...
    document.getElementById('wins').innerHTML = cu_wins;
    document.getElementById('losses').innerHTML = cu_losses;
}
...
function loadPlayersPage() {
    for (let i = 0; i < players.length; i++) {
        ...
        tag.setAttribute('onclick', `switchPlayers(${i})`);
        document.getElementById('player_selector').appendChild(tag);
    }
}
...
function switchPlayers(playerNum) {
    ...
    document.getElementById('avg_rec_yards').innerHTML =
