...
function getWeather() {
  ...
  //https://api.weatherstack.com/forecast?access_key={key}&query={latitude},{longitude}&forecast_days={number_of_days}
  var url = `https://api.weatherstack.com/forecast?access_key=5bc82451636190abd9d7afe6fe9b20b5&query=${latitude},${longitude}&forecast_days=5`;
  ...
  $.ajax({ url: url, dataType: "jsonp" }).then(function (data) {
    ...
    /*
      ...
      1. image_today : This should display an image for today's weather.
                ...
                */
    $("#local_time").html(Date(data.location.localtime))
    $("#image_today").attr("src", data.current.weather_icons[0])
    $("#heading").html("Weather Forecast - " + data.location.name)
    $("#temp_today").html(getFarenheitTemp(data.current.temperature))
    $("#thermometer_inner").css("height", `${getFarenheitTemp(data.current.temperature)}%`)
    if (getFarenheitTemp(data.current.temperature) >= 85) {
      $("#thermometer_inner").css("background-color", "red")
    } else if (getFarenheitTemp(data.current.temperature) <= 65) {
      $("#thermometer_inner").css("background-color", "blue")
    } else {
      $("#thermometer_inner").css("background-color", "grey")
    }
    $("#precip_today").html(`${data.current.precip * 100}%`)
    ...
    */
    ...
    for (let i = 0; i < 5; i++) {
      var today = getKey(i)
      final += `<div style="width: 20%;">
                  <div class="card">
                    <div class="card-body">
                      <h5 class="card-title">${dayOfWeek}</h5>
                      <p class="card-text">High: ${getFarenheitTemp(today.maxtemp)} <br>
                        Low: ${getFarenheitTemp(today.mintemp)}<br>
                        Sunrise: ${today.astro.sunrise}<br>
                        Sunset: ${today.astro.sunset}</p>
                    </div>
                  </div>
                </div>`
    }
    $("#5_day_forecast").html(final)
  })
};
